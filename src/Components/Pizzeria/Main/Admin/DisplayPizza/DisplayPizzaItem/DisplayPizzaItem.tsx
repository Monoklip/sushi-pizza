...
const DisplayPizzaItem = (props: { elem: { name: string; price: number; gramm: number; basket: string; image: string; id: number; }; getFood: any; }) => {
    ...
    const [url, setUrl] = useState<string>(`http://localhost:3000/pizza/${id}`);
            ...
            {displayDelete &&
                <div className='display-pizza-item'>
                    ...
                    <div className="display-pizza-item-buy">
                        <button className="display-pizza-item-buy-btnDelete" onClick={deleteBtn}>Удалити</button>
                        <button className="display-pizza-item-buy-btnRename" onClick={renameBtn}>Редагувати</button>
                    </div>
                </div>
            }
            {displayRename && 
                <div className='display-pizza-item-rename'>
                    <h1>Редагування</h1>
                    <p>Введіть назву</p>
