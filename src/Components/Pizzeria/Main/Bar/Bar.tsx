...
const Bar = (props: { uptadeFoodkList: (arg0: { name: string; price: number; image: string; gramm: number; num: number; sum: number; }) => void; }) => {
        ...
        <div className='bar'>
            ...
            <div className="bar-items">
                {bar.map((elem: { name: string; price: number; gramm: number; basket: string; image: string; num: number; id: number;}) => {
                    return <BarItem elem={elem} key={elem.id} uptadeFoodkList={props.uptadeFoodkList}/>
                })}
            </div>
        </div>
    )
